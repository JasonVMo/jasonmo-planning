...
1. Read AGENTS.md, ownership/evidence rules, affected data/view specs, taxonomy,
   the relevant owner research profile, and
   `research/topics/<id>/state.yaml`.
...
Record only a safe error category, affected source IDs, and next action. Do not
fabricate facts or timestamps, weaken validation, or store raw logs.

## Example

Resume a trip research topic, verify one primary source, and propose only its
managed summary and evidence updates.
